...
- i18n (pt-BR/en) server-driven (cookie httpOnly) sem estado client divergente
- SEO tecnico (robots/sitemap/rss + noindex consistente)
- E2E deterministico (sem flakiness, sem dependencias externas por padrao, matriz completa via PW_FULL)

## Mudancas feitas

- i18n (server-driven):
  - `src/lib/locale-server.ts`: `getRequestLocale()` memoizado por request (`react/cache`) e fallback `Accept-Language`.
  ...
  - `src/components/language-switcher.tsx`: troca via Server Action + `router.refresh()`; rollback do estado otimista em falhas.
...
- SEO tecnico:
  ...
  - `app/sitemap.ts`: sitemap apenas de paginas publicas + posts; tags entram so se tiverem >= 2 posts (evita thin content).
  - `app/robots.ts`: robots.txt minimalista (hint de crawl), sem depender de disallow para noindex.
  - `app/rss.xml/route.ts`: RSS do blog.
  - `app/layout.tsx`: canonical global, RSS discovery, e suporte a verificacao (Google/Bing) via env.
...
- Testes:
  ...
  - `e2e/i18n.e2e.ts` (troca de idioma + persistencia em todos os engines, incluindo mobile Safari).
  - `e2e/protected.e2e.ts` (cookie pt-BR em /login + fluxo real de signup).
