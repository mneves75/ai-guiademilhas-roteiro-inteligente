...
const AirplaneIcon = () => (
  <svg
    ...
    className="h-5 w-5 transform rotate-[-45deg]"
    fill="none"
    viewBox="0 0 24 24"
    stroke="currentColor"
  >
...
const extractFlightData = (markdown: string): FlightDeal[] => {
  ...
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (
        lines[i].includes('|') &&
        lines[i].toLowerCase().includes('destino') &&
        lines[i].toLowerCase().includes('cash')
      ) {
        tableStartIndex = i;
        break;
      }
    ...
    if (tableStartIndex !== -1) {
      const headerCells = lines[tableStartIndex]
        .split('|')
        .map((c) => c.trim())
        .filter((c) => c);
      ...
      const idxDur = headerCells.findIndex(
        (h) => h.toLowerCase().includes('duração') || h.toLowerCase().includes('conexões')
      );
  ...
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (
        lines[i].includes('|') &&
        lines[i].toLowerCase().includes('destino') &&
        lines[i].toLowerCase().includes('clima')
      ) {
        tableStartIndex = i;
        break;
      }
      ...
      if (destIndex !== -1 && climateIndex !== -1) {
        for (let i = tableStartIndex + 2; i < lines.length; i++) {
          ...
          if (cells[destIndex] && cells[climateIndex]) {
            ...
            if (
              lowerText.includes('risco alto') ||
              lowerText.includes('furacão') ||
              lowerText.includes('tempestade') ||
              lowerText.includes('extremo')
            ) {
              risk = 'high';
            } else if (
              lowerText.includes('risco médio') ||
              lowerText.includes('atenção') ||
              lowerText.includes('chuva frequente')
            ) {
              risk = 'medium';
            }
    ...
    console.error('Failed to parse climate data', e);
  }
  return data;
};

// Helper to extract destination guides and clean markdown
const extractGuidesAndCleanReport = (
  markdown: string
): { cleanReport: string; guides: DestinationGuide[] } => {
...
const BudgetWarning: React.FC<{ userBudget: number; estimatedCost: number }> = ({
  userBudget,
  estimatedCost,
}) => {
      ...
      <div className="flex items-start">
        ...
        <div className="ml-3">
          ...
          <div className="mt-2 text-sm text-amber-700">
            <p>
              O custo estimado no relatório (aprox.{' '}
              <strong>R$ {estimatedCost.toLocaleString('pt-BR')}</strong>) excede o seu orçamento
              alvo de <strong>R$ {userBudget.toLocaleString('pt-BR')}</strong>.
            </p>
            <p className="mt-1">
              Considere ajustar as datas, reduzir a categoria da hospedagem ou optar por voos com
              escalas para reduzir os custos.
            </p>
          </div>
        </div>
      </div>
  ...
  const getRiskStyles = (level: string) => {
    switch (level) {
      case 'high':
        return {
          container:
            'bg-red-50 border-red-200 hover:border-red-300 hover:shadow-lg hover:shadow-red-100/50',
          text: 'text-red-900',
          bar: 'bg-red-500',
      ...
      case 'medium':
        return {
          container:
            'bg-orange-50 border-orange-200 hover:border-orange-300 hover:shadow-lg hover:shadow-orange-100/50',
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">
            Análise de condições meteorológicas para o período
          </p>
        </div>
      </div>
          ...
          return (
            ...
            >
              <div className="flex justify-between items-start mb-3">
                ...
                {item.temperature && (
                  <span
                    className={`px-2.5 py-1 rounded-md text-sm font-bold shadow-sm border ${styles.badge}`}
                  >
                    {item.temperature}
                  </span>
                )}
              ...
              <div className="flex justify-between items-center mt-2">
                <span
                  className={`text-[10px] font-bold uppercase tracking-widest opacity-70 ${styles.text}`}
                >
                  Nível de Risco
                </span>
                <span className={`text-xs font-bold uppercase ${styles.text}`}>{styles.label}</span>
              </div>
            </div>
          );
        })}
    ...
    if (tableHeader.length > 0 && tableRows.length > 0) {
      elements.push(
        <div
          ...
          className="overflow-x-auto my-4 rounded-lg shadow-sm border border-gray-200"
        >
              ...
              {tableRows.map((row, i) => (
                <tr key={i} className="hover:bg-gray-50 transition-colors">
                  {row.map((cell, j) => (
                    <td
                      key={j}
                      className="px-6 py-4 whitespace-normal text-sm text-gray-700 max-w-xs"
                    >
                      {cell.trim()}
                    </td>
                  ))}
  ...
  lines.forEach((line, index) => {
    ...
    if (line.startsWith('# ')) {
      ...
      elements.push(
        <h1
          key={index}
          className="text-3xl font-bold mt-8 mb-4 pb-2 border-b border-gray-200 text-indigo-900"
        >
          {line.substring(2)}
        </h1>
      );
      ...
      flushTable();
      elements.push(
        <h3 key={index} className="text-xl font-semibold mt-5 mb-2 text-gray-700">
          {line.substring(4)}
        </h3>
      );
    }
      ...
      elements.push(
        <li key={index} className="ml-5 list-disc text-gray-700 mb-1 pl-1 marker:text-indigo-400">
          {line
            ...
            .replace(
              /\*\*(.*?)\*\*/g,
              (match, p1) => `<strong class="text-gray-900">${p1}</strong>`
            )
            .split(/<|>/)
            .map((chunk, i) => {
...
const ReportDisplay: React.FC<ReportDisplayProps> = ({ data, preferences }) => {
  ...
  const { cleanReport, guides } = useMemo(
    () => extractGuidesAndCleanReport(reportRaw),
    [reportRaw]
  );
  ...
  const userBudget = useMemo(
    () => parseCurrency(preferences.orcamento_brl),
    [preferences.orcamento_brl]
  );
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-10 max-w-none border border-gray-100">
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5 mb-8 border-b border-gray-100 pb-6">
        ...
        <div>
          ...
          <p className="text-sm sm:text-base text-gray-500 mt-1">
            Planejamento personalizado gerado por IA.
          </p>
        </div>
      </div>
      ...
      {/* Main Content */}
      <div className="prose prose-indigo prose-lg text-gray-600">{reportElements}</div>
