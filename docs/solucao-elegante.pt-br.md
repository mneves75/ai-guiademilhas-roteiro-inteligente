...
### 5) Contrato de API do planner (v2)

- Resposta de sucesso agora e versionada:
  - `{ schemaVersion, generatedAt, report, mode }`
- Rate limit usa `application/problem+json` (RFC 9457) com:
  - `type`, `title`, `status`, `detail`, `instance`, `requestId`, `code`, `retryAfterSeconds`
- Frontend faz parse resiliente para:
  - payload v2 (novo) e payload legado (sem quebrar compatibilidade retroativa)
